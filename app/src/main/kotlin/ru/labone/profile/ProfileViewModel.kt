...
class ProfileViewModel @AssistedInject constructor(
    ...
    val effects: Effects<ProfileEffects> = effects()
        ...
        profileRepository.flowCustomerProfile()
            .execute { profile: Async<CustomerProfile?> ->
                if (profile is Success) {
                    ...
                    effects.publish(RenderPhoto(details?.customerAvatar))
                    copy(
                        id = details?.id,
                        name = details?.name,
    ...
    fun onPickAvatar(uri: URI) = viewModelScope.launch {
        ...
        effects.publish(RenderPhoto(uri))
...
data class RenderPhoto(val photoUri: URI? = null) : ProfileEffects()
